...
```{r join-data-frames}
  ...
  #Should we instead to `PCBs_loc <- full_join(Texture_loc, PCBs2002, by = "UNIQUE_ID")` to include textures?
  
  PAHs_loc <- full_join(Station2002, PAHs2002, by = "UNIQUE_ID")
  #Should we instead to `PCBs_loc <- full_join(Texture_loc, PAHs2002, by = "UNIQUE_ID")` to include textures?
  
  Organics_loc <- full_join(Station2002, Organics2002, by = "UNIQUE_ID")
  #Should we instead to `PCBs_loc <- full_join(Texture_loc, Organics2002, by = "UNIQUE_ID")` to include textures?
...
```
`View(pcbs_pivot_longer)` is commented out as it is too large to load when knitting file.
