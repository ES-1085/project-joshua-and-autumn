...
Since these are all very large data files, we will select useful variables and variables we are interested in looking at to pare them down to a more manageable size.  Note that the `inorganics_full` data frame is commented out because we are unlikely to use it in our final analysis.
...
#Joining Dataframes
###Associates location information and contaminant data by sample unique ID. 
```{r join-data-frames}
