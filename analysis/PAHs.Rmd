...
``` {r Chrysene-concentration}
PAHs %>%
  filter(pah_detected == "CHRYSENE") %>%
  filter(amount_detected <= 2000, 
         amount_detected > 0) %>%
  ggplot(aes(x = amount_detected)) + 
  geom_histogram(binwidth = 50, fill="#69b3a2", color="#e9ecef", alpha=0.9) + 
   theme_minimal() +
  labs(title = "Chrysene Concentration (ng/g)", 
        x = "Amount Detected (ng/g)", 
        y = "Frequency of Amount Detected")
```

### Phenanthrene Concentration
``` {r Phenanthrene-concentration}
PAHs %>%
  filter(pah_detected == "PHNANTHR") %>%
  filter(amount_detected <= 2000, 
         amount_detected > 0) %>%
  ggplot(aes(x = amount_detected)) + 
  geom_histogram(binwidth = 50, fill="#69b3a2", color="#e9ecef", alpha=0.9) + 
   theme_minimal() +
  labs(title = "Phenanthrene Concentration (ng/g)", 
        x = "Amount Detected (ng/g)", 
        y = "Frequency of Amount Detected")
```
