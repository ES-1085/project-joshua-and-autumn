...
PAHs_station %>%
  ...
  filter(amount_detected < 30000) %>%
  ggplot(aes(x = SOUNDING_M.x, y = amount_detected, color = pah_detected))+
  geom_point()+
...
PAHs_station %>%
  ...
  coord_flip()+
  theme(legend.position = "none")+
  labs(title = "PAHs Detected in Gulf of Maine",
       x = "PAH Detected",
       y = "Amount Detected")
...
``` {r sum-organics-boston-central-harbor}
PAHs_station %>%
  group_by(GEN_LOC_NM.x, pah_detected) %>%
  summarise(amount_detected = sum(amount_detected, na.rm = TRUE)) %>%
  filter(GEN_LOC_NM.x == "CENTRAL BOSTON HARBOR") %>%
  arrange(desc(amount_detected))

PAHs_station %>%
  drop_na(amount_detected) %>%
  filter(GEN_LOC_NM.x %in% c("CENTRAL BOSTON HARBOR")) %>%
 ggplot(aes(x = fct_rev(fct_reorder(pah_detected, amount_detected)), y = amount_detected, fill = pah_detected))+
  geom_col()+
  coord_flip()+
  theme(legend.position = "none")+
...
Rivers_Summary_PAHs <- PAHs_station %>%
...
Rivers_Summary_PAHs %>%
  filter(SPECFC_LOC.x %in% c("Fore River", "Kennebunk River", "Royal River", "Kennebec River", "Scarborough River", "Saco River", "Machias River", "Union River", "Penobscot River", "Harraseeket River (Freeport Harbor)", "York River", "Hardscrabble River", "St. George River")) %>%
  ggplot(aes(x = fct_rev(fct_reorder(SPECFC_LOC.x, mean_PAH)), y = sapply(mean_PAH, FUN=function(x) ifelse(x==0.000000e0, -0.01,x) ), fill = SPECFC_LOC.x))+
...
## Kennebunk River Concentrations
This plot visualizes the specific PAHs present in Kennebunk River.  
As shown in the plot, Indeno Pyrene (1250	ng/g) has the highest concentration. This is followed by Dibenz (a,h) Anthracene (1110 ng/g), and Benzo floranthene (620 ng/g). 
```{r type_of_pah-in-kennebunk, warning = FALSE}
PAHs_station %>%
  group_by(SPECFC_LOC.x, pah_detected) %>%
  summarise(amount_detected = sum(amount_detected, na.rm = TRUE)) %>%
  filter(SPECFC_LOC.x == "Kennebunk River") %>%
  arrange(desc(amount_detected))

PAHs_station %>%
  drop_na(amount_detected) %>%
  filter(SPECFC_LOC.x %in% c("Kennebunk River")) %>%
 ggplot(aes(x = fct_rev(fct_reorder(pah_detected, amount_detected)), y = amount_detected, fill = pah_detected))+
  geom_col()+
  coord_flip()+
  theme(legend.position = "none")+
  labs(title = "PAH Concentrations in Kennebunk River",
       x = "PAH Detected",
       y = "Amount Detected (ng/g)")
```

## York River Concentrations
This plot visualizes the specific PAHs present in Kennebunk River.  
As shown in the plot, Fluoranthene (150	ng/g) has the highest concentration. This is followed by Pyrene (130 ng/g) and Benzo Pyrene (84 ng/g). 
```{r type_of_pah-in-york, warning = FALSE}
PAHs_station %>%
  group_by(SPECFC_LOC.x, pah_detected) %>%
  summarise(amount_detected = sum(amount_detected, na.rm = TRUE)) %>%
  filter(SPECFC_LOC.x == "York River") %>%
  arrange(desc(amount_detected))

PAHs_station %>%
  drop_na(amount_detected) %>%
  filter(SPECFC_LOC.x %in% c("York River")) %>%
 ggplot(aes(x = fct_rev(fct_reorder(pah_detected, amount_detected)), y = amount_detected, fill = pah_detected))+
  geom_col()+
  coord_flip()+
  theme(legend.position = "none")+
  labs(title = "PAH Concentrations in York River",
       x = "PAH Detected",
       y = "Amount Detected (ng/g)")
```
