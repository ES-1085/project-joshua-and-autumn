...
### Fluoranthene Concentration
 filter(pah_detected %in% c("FLORNTHN", "PYRENE", "CHRYSENE", "PHNANTHR")) %>%

``` {r fluoranthene-concentration}
#check out the "BENZNE" and "X2BZFRN"... interesting!
#PAHs %>%
  #filter(pah_detected %in% c("FLORNTHN", "PYRENE", "CHRYSENE", "PHNANTHR", "BENZNE", "X2BZFRN")) %>%
  #filter(amount_detected <= 1000 ) %>%
...
#PAHs %>%
  #filter(pah_detected %in% c("FLORNTHN")) %>%
  #filter(amount_detected >= 1) %>%
#ggplot(aes(x = LONGITUDE, y = LATITUDE, col = pah_detected))+
  #geom_hex()+
  #theme_bw()
```

``` {r trying to animate, ignore this}
#PAHs %>%
  #filter(pah_detected %in% c("FLORNTHN")) %>%
  #filter(amount_detected >= 1) %>%
  #ggplot(aes(x = LONGITUDE, y = LATITUDE, size = amount_detected, fill = amount_detected)) + 
  #geom_point() + 
  #scale_x_log10() + 
  #theme_minimal() + 
  #labs(title = "Year: {SAMP_DATE1}", x = "Longitude", y = "Latitude") + 
