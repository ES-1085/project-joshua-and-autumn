...
As shown in the plot, CHRYSENE_C has the highest concentration in the harbor. This is followed by BZ_A_ANT_C, PYRENE_C, and FLORNTHN_C. 
...
PAHs_station %>%
  ...
  summarise(amount_detected = sum(amount_detected, na.rm = TRUE)) %>%
  filter(GEN_LOC_NM.x == "BOSTON INNER HARBOR") %>%
  ggplot(aes(x = pah_detected, y = amount_detected, fill = pah_detected))+
  geom_col()+
  coord_flip()+
  theme(legend.position = "none")+
...
As shown in the plot, [...] has the highest concentration in the harbor. 
```{r type_of_organic-in-BOSTON-INNER-HARBOR, warning = FALSE}
PAHs_station %>%
  drop_na(amount_detected) %>%
  filter(GEN_LOC_NM.x %in% c("BOSTON INNER HARBOR")) %>%
  ggplot(aes(x = pah_detected, y = amount_detected, fill = pah_detected))+
  geom_col()+
  coord_flip()+
  theme(legend.position = "none")+
...
PAHs_station %>%
  filter(GEN_LOC_NM.x %in% c("BOSTON INNER HARBOR")) %>%
  summarise(site = GEN_LOC_NM.x, 
            pah_detected = pah_detected, 
            sum_pah_detected = sum(amount_detected, na.rm = TRUE))
```
