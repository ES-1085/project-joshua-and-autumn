...
Sum_Org_site %>%
  ...
  c("Kennebunk River", "Royal River", "Kennebec River to Bath", "Portland Harbor & Fore River", "Saco River & Camp Ellis Harbor", "Machias River", "Union River", "York River", "MOUTH PENOBSCOT RIVER", "PENOBSCOT RIVER", "KENNEBEC RIVER", "KENNEBEC RIVER,ME."< "Fore River", "St. George River", "Saco River"))
...
Organics_station %>%
  group_by(site.x, organics_detected) %>%
  summarise(amount_detected = sum(amount_detected, na.rm = TRUE)) %>%
  filter(site.x == c("York River",  "Kennebunk River", "Portland Fore River", "Royal River", "Kennebec River to Bath", "Penobscot River To Bangor")) %>%
  arrange(desc(amount_detected))
```
