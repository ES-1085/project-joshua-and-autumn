...
Organics_long <- Organics %>%
 pivot_longer(cols = `REPNO_ORG`:`TTBT`, 
...
```{r pcbs-pivot-long}
PCBs_long <- PCBs %>%
  pivot_longer(cols = `PCB_52`:`LINDANE`, 
               names_to = "pcb", 
               values_to = "amount_detected")
...
pesticides <- PCBs_long %>%
  mutate(pcb = fct_recode(pcb,
                          ...
                          "BHC" = "BHC_B",
                          "BHC" = "BHC_D")) %>%
  filter(pcb %in% c("DDT", "DDT_4_4", "DDE_4_4", "DDD_4_4", "ENDRIN", "ENDR_ALD", "ALDRIN", "DIELDRN", "CLRDNE_T", "MIREX", "METHOXYCL", "BHC", "LINDANE", "BHC"))
...
```{r}
pcbs_only <- PCBs_long %>%
  filter(pcb %in% c("PCB_52", "PCB101", "PCB118", "PCB128", "PCB138", "PCB153", "PCB180", "PCB206", "PCB209"))
...
Clostridium perfringens (724,143.53 spores/g) is PCB with the highest concentration.
...
#labels <- sprintf("<strong>%s</strong><br/>%s %g ug/g</strong><br/>s %g ug/g</strong><br/>",
#                  pesticides_no_na_no_zero$SPECFC_LOC, pesticides_no_na_no_zero$pcb == "BHC_C", pesticides_no_na_no_zero$amount_detected, #pesticides_no_na_no_zero$pcb == "DDT_C", pesticides_no_na_no_zero$amount_detected) %>%
...
markers <- markers %>%
  mutate(labels = paste0("<strong>", "Site: ", site,"</strong><br>",
                         "BHC:", amount_detected[pcb == "BHC_C"], "<br>",
                         "Chlordane:", amount_detected[pcb == "CLRDNE_T_C"], "<br>",
                         "DDT:", amount_detected[pcb == "DDT_C"], "<br>",
                         "DDE:", amount_detected[pcb == "DDE_4_4_C"], "<br>",
                         "DDD:", amount_detected[pcb == "DDD_4_4_C"], "<br>",
                         "Aldrin:", amount_detected[pcb == "ALDRIN_C"], "<br>",
                         "Dieldrin:", amount_detected[pcb == "DIELDRN_C"],
                         "Endrin:", amount_detected[pcb == "ENDRIN_C"], "<br>",
                         "Endrin Aldehyde:", amount_detected[pcb == "ENDR_ALD_C"], "<br>",
                         "Lindane:", amount_detected[pcb == "LINDANE_C"], "<br>",
                         "Mirex:", amount_detected[pcb == "MIREX_C"], "<br>",
                         "Methoxychlor:", amount_detected[pcb == "METHOXYCLC"], "<br>"))


head(markers$labels, 3)
```
":", amount_detected[pcb == ""], "<br>",
