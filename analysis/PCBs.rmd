...
# Joining the Station Information to the PCB dataset
Joining the station data with the PCB sample taken. The dataset is pivoted so that each observation is the PCB that was observed.
```{r joining-station-info, warning=FALSE}
PCBs_station <- full_join(Station2002, PCBs, by = "UNIQUE_ID")

PCBs_station <- PCBs_station %>%
  pivot_longer(cols = `PCB_52_NGG`:`LINDANE_C`, 
               names_to = "pcbs_detected", 
               values_to = "amount_detected")

PCBs_station = select(PCBs_station, c(UNIQUE_ID, LATITUDE.x, LONGITUDE.x, SOUNDING_M.x, QUAD_NAME.x, GEN_LOC_NM.x, SPECFC_LOC.x, AREA_CODE.x, SAMP_DATE1.x, TO_SMP_DT2.x, DPTH_N_COR.x, DPTH_CODE.x, COR_GRB_CD.x, site.x, pcbs_detected, amount_detected))

#glimpse(PCBs_station)
```

## Visualizations
### Descriptive Visualizations
Visualizing the number of observations that were collected for each general location. 
```{r n-obs-gen-loc}
Organics %>%
