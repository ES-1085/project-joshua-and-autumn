...
Organics <- Organics %>%
  mutate(missing = case_when(is.na(PCB_T_UGG) == TRUE ~ "missing",
                             TRUE ~ "not missing"),
         size = case_when(is.na(PCB_T_UGG) == TRUE ~ 0.1,
                             TRUE ~ 0.3))

ggplot(GOM_states) +
  geom_sf(aes()) +
  geom_sf(data = Bathy_low_res, color = "gray80", width = 1)+
  geom_point(data = Organics, aes(x = LONGITUDE, y = LATITUDE, color = PCB_T_UGG, shape = missing, size = missing), alpha = 0.5) +
  xlim(-72,-65)+
  ylim(40,45)+
  theme_bw()+
...
Org_no_na_no_zero <- Org_no_na_no_zero %>%
  drop_na(SOUNDING_M) %>%
  filter(SOUNDING_M != "0")
  
labels <- sprintf("<strong>%s</strong><br/>%g ug/g</strong><br/>%g Depth M",
                  Org_no_na_no_zero$SPECFC_LOC, Org_no_na_no_zero$PCB_T_UGG, Org_no_na_no_zero$SOUNDING_M) %>%
...
```{r leaflet-map-pcb}
leaflet(data = Org_no_na_no_zero) %>%
