...
As stated in our project proposal, we were interested in visualizing contaminants in Maine Rivers as well as harbors and bays. The table below is visualizing the mean, standard deviation, count, and standard error values of PCB concentrations for river locations.
...
```{r bar-pcb-me-rivers-without-union, fig.alt = "Bar plot of mean total PCB concentrations in micrograms per gram of sediment from Maine Rivers.  The Portland Fore River has the highest total concentration (mean = 13.18 (+/- 43.72) ng/g). Kennebunk River has the second highest conentration (mean = 0.77 (+/- 1.54) ng/g).  No PCBs were detected in the Kennebec or Penobscot Rivers (indicated with negative bars).  The Union River has been excluded from this plot."}
Sum_Org_site %>%
...
Interpretation: The Portland Fore River has the highest total concentration (mean = 13.18 (+/- 43.72) ng/g). Kennebunk River has the second highest conentration (mean = 0.77 (+/- 1.54) ng/g).  No PCBs were detected in the Kennebec or Penobscot Rivers.  The Union River has been excluded from this plot.

```{r bar-pcb-me-rivers-with-union, fig.alt = "Bar plot of mean total PCB concentrations in micrograms per gram of sediment from Maine Rivers.  The Union River in Ellsworth, Maine has the highest total concentration (mean = 276.6 ng/g).  There is only one observation associated with the Union River.  The Portland Fore River has the second highest total concentration (mean = 13.18 (+/- 43.72) ng/g). Kennebunk River has the third highest conentration (mean = 0.77 (+/- 1.54) ng/g).  No PCBs were detected in the Kennebec or Penobscot Rivers (indicated with negative bars)."}
Sum_Org_site %>%
...
Intrepretation: The Union River in Ellsworth, Maine has the highest total concentration (mean = 276.6 ng/g).  There is only one observation associated with the Union River.  The Portland Fore River has the second highest total concentration (mean = 13.18 (+/- 43.72) ng/g). Kennebunk River has the third highest conentration (mean = 0.77 (+/- 1.54) ng/g).  No PCBs were detected in the Kennebec or Penobscot Rivers.  There are no errorbars for Union River as there is only one observation from this river.

```{r identiy-ma-rivers, echo = FALSE}
#Organics %>%
...
Intrepretation: The Chelsea River in Boston and Chelsea, Massachusetts has the highest concentration (mean = 57.08 (+/- 95.25) ng/g) -- this is a highly industrial area.  Island End River has the second highest concentration (mean = 9.52 (+/- 7.86) ng/g).  The Merrimack River and Mill Creek have no detected PCBs (indicated with negative bars).
...
*Note that the version included in our presentation contained two errors; we had originally copied the code from a general location comparison but had inadvertently retained the filter function such that the summary statistics were still being filtered for that general location.  Additionally, the summary statistics were grouped by specific location name and not per sample, resulting in higher mean values that per sample.  This is the corrected version.
...
``` {r type_of_organic-in-boston-inner-harbor-graph, warning = FALSE, fig.alt = "Pie chart showing composition of general organic contaminants in Boston Inner Harbor sediments.  PCBs are most abundant, followed by oil and greese.  DDT, PHC, and MBT comprise substantial proportions.  Clostridium sp. was detected."}
Organics_long_no_na_no_zero %>%
...
Interpretation: PCBs are most abundant, followed by oil and greese.  DDT, PHC, and MBT comprise substantial proportions.  Clostridium sp. was detected.
...
``` {r type_of_organic-in-cape-ann-to-elizabeth-graph, warning = FALSE, fig.alt = "Pie chart of composition of general organic contaminats in inshore area from Cape Ann, MA to Cape Elizabeth, ME. Clostridium sp. is by far the most prevalent contaminant at this location.  PCB totals comprise a significantly smaller proportion, and are followed by oil and greese."}
Organics_long_no_na_no_zero %>%
...
Intrepretation: Clostridium sp. is by far the most prevalent contaminant at this location.  PCB totals comprise a significantly smaller proportion, and are followed by oil and greese.
...
``` {r type_of_organic-in-boston-southeast-harbor-graph, warning = FALSE, fig.alt = "Pie chart of general organic composition in Southeast Boston Harbor.  Clostridium sp. accounts for slightly more than half the detected contaminants.  Oil and greese and PHC also make up significant proportions."}
Organics_long_no_na_no_zero %>%
...
Interpretation: Clostridium sp. accounts for slightly more than half the detected contaminants.  Oil and greese and PHC also make up significant proportions.
...
Organics_long_no_na_no_zero %>%
  ...
  arrange(desc(amount_detected))
```
``` {r type_of_organic-from-cape-elizabeth-to-rockland-graph, warning = FALSE, fig.alt = "Pie chart of general organic contaminant composition from Cape Elizabeth, ME, to Rockland, ME.  DDT constitutes the majority of general organics detected in this zone.  PCBs also make up a much smaller proportion  Clostridium sp. is not detected here."}
Organics_long_no_na_no_zero %>%
...
Interpretation: DDT constitutes the majority of general organics detected in this zone.  PCBs also make up a much smaller proportion  Clostridium sp. is not detected here.
...
Organics_long_no_na_no_zero %>%
  ...
  arrange(desc(amount_detected))
```
``` {r type_of_organic-in-rockalnd-to-N-graph, warning = FALSE, fig.alt = "Pie chart of general organic contaminant composition from Rockland to Bay of Fundy.  DDT makes up greater than 75% of total; PCBs make up less than 25%.  Clostridium sp. was not detected in this zone."}
Organics_long_no_na_no_zero %>%
...
Interpretation: DDT makes up greater than 75% of total; PCBs make up less than 25%.  Clostridium sp. was not detected in this zone.
...
```{r map-all-stations, fig.alt = "Map of all sampled locations in the Gulf of Maine. Sampling intensity was greatest inshore, along the coast from Cape Cod Bay to the Bay of Fundy. Offshore sampling mostly was done in a grid pattern; there are some clumps of apparently random points throughout the offshore area, especially along the continental shelf southeast of Cape Cod."}
ggplot(GOM_states) +
  geom_sf(aes()) +
...
Interpretation: Sampling intensity was greatest inshore, along the coast from Cape Cod Bay to the Bay of Fundy. Offshore sampling mostly was done in a grid pattern; there are some clumps of apparently random points throughout the offshore area, especially along the continental shelf southeast of Cape Cod.
...
Interpretation: The highest PCB concentrations are in Boston Harbor and along the New Hampshire coast.  Detection density is highest in Massachusetts Bay.  Other locations are sparsely scattered along the Maine coast and offshore.
...
Interpretation: The highest PCB concentrations are in the Union River, Ellsworth, and in Bass Harbor. Some low concentrations were detected along the west shore of Penobscot Bay.
...
Interpretation: Highest PCB concentrations are in Boston Harbor, and at Hampton/Seabrook, NH.  Highest density of detections is in basin of Massachusetts Bay outside of Boston Harbor.
...
```{r map-gen-org-gom, fig.alt = "Map of central New England Coast, with points indicating stations where general organic contaminants were detected.  Color of points indicates contaminant detected.  Clostridium sp. is abundant from Cape Cod Bay to the mouth of Piscataqua River in New Hampshire/ME."}
ggplot(GOM_states) +
  geom_sf(aes()) +
  geom_sf(data = Bathy_low_res, color = "gray80", width = 1) +
...
Interpretation: Clostridium sp. is abundant from Cape Cod Bay to the mouth of Piscataqua River in New Hampshire/ME.

```{r map-all-pesticides-gom, warning = FALSE, fig.alt = "Map of the Gulf of Maine, with points indicating pesticide detections.  Points are colored by pesticide type; point radius indicates concentration.  The highest concentrations and diversity of pesticides are found in and around Boston Harbor, Massachusetts, with dieldrin and endrin aldehydr being highly concentrated.  The Maine coast sees a much lower diversity of pesticides, primarily Lindane and BHC, with some Chlordane."}
ggplot(GOM_states) +
  geom_sf(aes()) +
  geom_sf(data = Bathy_low_res, color = "gray80", width = 1) +
...
Interpretations: The highest concentrations and diversity of pesticides are found in and around Boston Harbor, Massachusetts, with dieldrin and endrin aldehydr being highly concentrated.  The Maine coast sees a much lower diversity of pesticides, primarily Lindane and BHC, with some Chlordane.

```{r map-pest-mass-bays, fig.alt = "Map of Boston Harbor and part of Massachusetts Bay with points indicating pesticide detections.  Points are colored by pesticide type; point radius indicates concentration.  Dieldrin, endrin aldehyde, and endrin are principally found near shore, while lindane, BHC, and DDT and its products are found further offshore."}
ggplot(GOM_states) +
  geom_sf(aes()) +
  geom_sf(data = Bathy_hi_res, color = "gray80", width = 1) +
  geom_point(data=pesticides, (aes(x = LONGITUDE, y = LATITUDE, size = amount_detected, color = pcb, alpha = 0.5))) +
...
Interpretation: Dieldrin, endrin aldehyde, and endrin are principally found near shore, while lindane, BHC, and DDT and its products are found further offshore.
...
```{r leaflet-map-pesticides, fig.alt = "Interactive map of pesticide detections in the Gulf of Maine.  Each point indicates where at least one pesticide was detected.  Clicking on a point opens a pop-up window that gives a list of concentrations of each detected pesticide."}
#leaflet() %>%
...
```{r leaflet-map-pcbs, fig.alt = "Interactive map of PCB detections in the Gulf of Maine.  Each point indicates where at least one PCB was detected.  Clicking on a point opens a pop-up window that gives a list of concentrations of each detected PCB."}
#leaflet() %>%
#  addProviderTiles(providers$Esri.WorldTopoMap) %>%
#  setView(lng = -68.5, 
#          lat = 43.5, 
