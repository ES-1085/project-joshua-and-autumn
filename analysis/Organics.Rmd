...
This plot visualizes the specific organics present in Boston Central Harbor. As shown in the plot, there is a high concentration of Oil and Grease in sediments (in units of moles/g), Petroleum Hydrocarbons (in units of percent dry weight), and Clostridium perfringens (in units of spores/g). 
``` {r type_of_organic-in-boston-central-harbor, warning = FALSE}
Organics_station %>%
  filter(site %in% c("Boston Central Harbor")) %>%
  ggplot(aes(x = organic_detected, y = amount_detected, fill = organic_detected))+
  geom_col(col = "black")+
  coord_flip()+
  theme(legend.position = "none")+
